"""Fix order_tasks table - add missing task_uuid and user_name fields

Revision ID: 9a9963e9830f
Revises: 
Create Date: 2025-07-31 15:46:06.340381
...
def upgrade() -> None:
    ...
    # 只添加缺失的字段到 order_tasks 表
    op.add_column('order_tasks', sa.Column('task_uuid', sa.String(length=36), nullable=False, comment='任务唯一标识UUID'))
    op.add_column('order_tasks', sa.Column('user_name', sa.String(length=100), nullable=False, comment='用户名'))
    op.create_unique_constraint('uq_order_tasks_task_uuid', 'order_tasks', ['task_uuid'])
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'order_tasks', type_='unique')
    op.drop_column('order_tasks', 'user_name')
    ...
    sa.Column('created_at', mysql.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
    sa.Column('updated_at', mysql.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), nullable=True),
    sa.ForeignKeyConstraint(['platform_id'], ['platforms.id'], name=op.f('shops_ibfk_1')),
    sa.PrimaryKeyConstraint('id'),
    comment='åº—é“ºè¡¨',
    mysql_collate='utf8mb4_0900_ai_ci',
